...
import { getDepositTransactions } from "../../actions/transactionAction";
...
const filter_field = ["updated_at", "amount", "status", "profit"];
...
function Dashboard({
  ...
  useEffect(() => {
    ...
    const availableBalance = Math.fround(
      reflectBalanceState.reflectuserBalances[0].availableBalance
    ).toFixed(2);
    const depositBalance = Math.fround(
    ...
    setStatisticsInfo({
      ...
      availableBalance: Math.fround(
        reflectBalanceState.reflectuserBalances[0].availableBalance
      ).toFixed(2),
      profitAmount: Math.fround(
        reflectBalanceState.reflectuserDeposits[0].profitAmount
      ).toFixed(2),
      depositBalance: Math.fround(
        reflectBalanceState.reflectuserDeposits[0].depositBalance
      ).toFixed(2),
      totalWithdrawed: Math.fround(
        reflectBalanceState.reflectuserWithdraws[0].totalWithdrawed
      ).toFixed(2),
      pendingWithdraw: Math.fround(
    ...
    setInfo({
      ...
      activeDeposit: Math.fround(
        reflectBalanceState.reflectuserDeposits[0].depositBalance
      ).toFixed(2),
      earnedProfit: Math.fround(
        reflectBalanceState.reflectuserDeposits[0].profitAmount
      ).toFixed(2),
    });
  }, [balance]);
  ...
  const fetchDepositTransactions = useCallback(
    ...
    [depositTransactions, getDepositTransactions, user]
  );
  ...
  }, [user, fetchDepositTransactions, getBalance]);
  ...
  return (
    <div className="dashboard">
      <div className="dashboard-header">
        ...
        />
        <Text className="caption H2 dark_1" text={"Dashboard"} />
      </div>
      <div className="dashboard-balance">
        <Text className="H3 dark_1" text={"Your Balances"} />
      ...
      <div className="dashboard-statistics">
        <Text className="H3 dark_1" text={"Account Statistics"} />
      ...
      <div className="dashboard-transactions">
        ...
        <div className="dashboard-transactions-content">
          ...
          <div className="table-content">
            {depositTransactions.models.map((item, _index) => (
              <div key={_index} className="table-content-item">
                {Object.entries(item).map(
                  (i) =>
                    filter_field.indexOf(i[0]) !== -1 && (
                      <div className="col" key={_index}>
                        {i[0] === "updated_at" ? (
                          <div>{displayDate(i[1])}</div>
                        ) : (
                          <div
                            key={_index}
                            className={`${i[0]} ${i[0] === "status" && i[1]}`}
                          >
        ...
        <div className="dashboard-transactions-footer">
          <Text
            ...
            text={`Showing 
              ${
                depositTransactions.pagination.pageSize *
                  (depositTransactions.pagination.page - 1) +
                1
              } 
              to 
              ${
                depositTransactions.pagination.pageSize *
                depositTransactions.pagination.page
              } 
            selected options`}
          />
          <Paginate
...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    getDepositTransactions: (payload) =>
      dispatch(getDepositTransactions(payload)),
  };
};
