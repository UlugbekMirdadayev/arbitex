...
function Home({ _mode, user, order, balance, getBalance, createOrder }) {
  const [orderShow, setOrderShow] = useState(false);
  // const [token, setToken] = useState(10);
  const [open, setOpen] = useState(false);
  const [mode] = useState(false);
  const [profit] = useState(0.5); //profit
  const [orderTime] = useState(2); //hour
  const [currency] = useState("ERC20USDT");
  const [currentBalance, setCurrentBalance] = useState(0);
  const [amount, setAmount] = useState(0);
  const [update, setUpdate] = useState(false);
  const [resultOpen, setResultOpen] = useState(false);
  // const [address, setAddress] = useState(
  //   "0x208af78a16731e0baeb47ee7c7dd7305b30232ce"
  // );
  ...
  useEffect(() => {
    ...
    setInfo({
      ...
      withdrawPending: Math.fround(
        reflectBalanceState.reflectuserWithdraws[0].pendingWithdraw
      ).toFixed(2),
    });
  }, [balance]);
  ...
  const done = (value) => {
    ...
    if (amount < orderAmount || amount > Math.floor(currentBalance)) {
      displayMsg(
        "Please make sure if your amount is greater than 500 and less than your balance.",
        "error"
      );
      return false;
    }
  ...
  return (
    <div className="home">
      <div className="home-header">
        ...
        />
        <Text className="caption H2 dark_1" text={"Home"} />
      </div>
      <div className="section">
        ...
        <div className="desc-section">
          <Text className={"title H3 dark_1"} text={"Investment package"} />
        ...
        <div className="help-section bg-dark_6">
          ...
          />
          {help.map((item, _index) => (
            <Text key={_index} className={`help-item P4 dark_2`} text={item} />
          ))}
        </div>
      </div>
      <Modal open={orderShow} onClose={() => setOrderShow(false)}>
        <div className="create-order">
          <Text className={"H3 dark_1"} text={"Create Order"} />
          <div className="item">
            <Text
          ...
          <div className="item">
            <Text className={"cap H6 dark_1"} text={"Currency: " + currency} />
          </div>
          <div className="item">
            <Text
              className={"cap H6 dark_1"}
          ...
          <div className="item">
            <Text className={"cap H6 dark_1"} text={"Amount: "} />
            <Input text={amount} onChange={(value) => setAmount(value)} />
          </div>
          <Button
      ...
      <Modal open={open} onClose={() => setOpen(false)}>
        <div className="create-order-modal">
          <div className="create-order-modal-header">
            <Text className={"H4 dark_1"} text={"Create Order"} />
          </div>
          <div className="create-order-modal-body">
            <div>
              <Text className={"P3 dark_2"} text={"Order Mode: "} />
              <Text className={"P3 success"} text={mode ? "Auto" : "Manual"} />
            </div>
            <div>
              <Text className={"P3 dark_2"} text={"Current balance: "} />
              <Text className={"P3 success"} text={currentBalance + " USDT"} />
            </div>
            <div>
              <Text className={"P3 dark_2"} text={"Currency: "} />
              <Text className={"P3 success"} text={currency} />
            </div>
            <div>
              <Text className={"P3 dark_2"} text={"Order Amount: "} />
              <Text className={"P3 success"} text={amount + " USDT"} />
            </div>
            {!mode && (
              <div>
                <Text className={"P3 dark_2"} text={"Order Time:"} />
                <Text className={"P3 success"} text={orderTime + " h"} />
              </div>
            )}
      ...
      <Modal open={resultOpen} onClose={() => setResultOpen(false)}>
        <div className="create-order-modal">
          <div className="create-order-modal-header">
            <Text className={"H4 dark_1"} text={"Result"} />
          </div>
          <div className="create-order-modal-body">
            <div>
              <Text
