...
import { getQRcode, getBalance } from "../../actions/accountAction";
...
import { createWithdrawFunds } from "../../actions/transactionAction";
...
const user_title = ["id", "email", "username", "publish"];
...
function Manage({
  ...
  const done = () => {
    ...
    if (amount <= 0) {
      displayMsg(
        "Amount can't be lower or equal than zero. Please try it to check !",
        "error"
      );
      return;
    }
    ...
    if (amount > companyBalance) {
      displayMsg(
        "Amount can't be bigger than your available balance. Please try it to check !",
        "error"
      );
      return;
    }
  ...
  return (
    <div className="manage">
      <div className="manage-header">
        ...
        />
        <Text className="caption H2 dark_1" text={"Manage"} />
      </div>
      <div className="manage-section">
        <div className="manage-user">
          <div className="manage-user-header">
            <Text className="H3 dark_2" text={"User Manage"} />
          </div>
          <div className="manage-user-content">
            <div className="table-title">
              {user_title.map((item, _index) => (
                <div key={_index} className="table-title-item">
                  {item}
                </div>
              ))}
            </div>
            <div className="table-content">
              {users.data.map((item, idx) => (
                <div className="table-content-item">
                  ...
                  <div className="col">
                    <div
                      className={`checkbox ${item["publish"] && "checked"}`}
                      onClick={() => {
        ...
        <div className="manage-platform">
          <div className="manage-platform-header">
            <Text className="H3 dark_2" text={"Platform Manage"} />
          </div>
          <div className="manage-platform-content">
            {platforms.map((item, _index) => (
              <Statistics
                key={_index}
                icon={item.icon}
      ...
      <div className="manage-transactions">
        ...
        <div className="manage-transactions-content">
          ...
          <div className="table-content">
            {profits.data.models.map((item, _index) => (
              <div key={_index} className="table-content-item">
                <div className="col">{displayDate(item["created_at"])}</div>
                <div className="col">{item["earned"].toFixed(2)}</div>
                <div className="col">{item["u_earned"].toFixed(2)}</div>
                <div className="col">{item["c_earned"].toFixed(2)}</div>
                <div className="col">{item["amount"].toFixed(2)}</div>
        ...
        <div className="manage-transactions-footer">
          <Text
            ...
            text={`Showing 
              ${
                profits.data.pagination.pageSize *
                  (profits.data.pagination.page - 1) +
                1
              } 
              to 
              ${
      ...
      <Modal open={open} onClose={() => setOpen(false)}>
        <div className="withdraw-funds-modal">
          <div className="withdraw-funds-modal-header">
            <Text className={"H4 dark_3"} text={"Confirm Withdraw"} />
          </div>
          <div className="withdraw-funds-modal-content">
            ...
            <Text
              className={"P3 dark_2"}
              text={
                "Copy your unique address or use the QR code to make a withdraw to wallet."
              }
            />
            <div className="img">
              <img src={qrcode.url} alt="qrcode.url" />
            </div>
            <div className="total-amount">
              <Text className={"P3 dark_2"} text={"your available balance: "} />
              <Text
            ...
            <div className="to-address">
              <Text className={"P3 dark_2"} text={"To Address:"} />
            </div>
            <Input
              classNames="H5 dark_4"
              text={address}
            ...
            <div className="verify-token">
              <Text className={"P3 dark_2"} text={"Verify Token: "} />
            </div>
            <Input
              classNames="H5 dark_4"
              text={token}
            ...
            <div className="amount">
              <Text className={"P3 dark_2"} text={"Amount"} />
            </div>
            <Input
              classNames="H5 dark_4"
              text={amount}
      ...
      <div className="manage-transactions">
        <div className="manage-transactions-header">
          <Text className="H3 dark_1" text={"COMPANY WITHDRAW"} />
        </div>
        <div className="manage-transactions-content">
          <div className="table-title">
            {withdraw_title.map((item, _index) => (
              <div className="table-title-item">{item}</div>
            ))}
          </div>
          <div className="table-content">
            {withdraws.data.models.map((item, _index) => (
              <div key={_index} className="table-content-item">
                <div className="col">{displayDate(item["created_at"])}</div>
        ...
        <div className="manage-transactions-footer">
          <Text
            ...
            text={`Showing 
              ${
                withdraws.data.pagination.pageSize *
                  (withdraws.data.pagination.page - 1) +
                1
              } 
              to 
              ${
                withdraws.data.pagination.pageSize *
                withdraws.data.pagination.page
              } 
            selected options`}
          />
          <Paginate pageCount={withdraws.data.pagination.pageCount} />
        </div>
      </div>
    </div>
  );
