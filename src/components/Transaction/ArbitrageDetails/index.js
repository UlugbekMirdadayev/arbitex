...
function ArbitrageDetails({ user, arbitrageDetails, getArbitrageDetails }) {
  ...
  const stopProcessor = (mode, timeLine, id, loading) => {
    ...
    return (
      <Button
        className={`disabled ${
          titleButton === "STOP" && mode === 0
            ? "bg-dark_5"
            : "bg-success_shade success"
        }`}
        onClick={() =>
          mode === 1 && titleButton === "STOP" && stopTransaction(id)
        }
  ...
  return (
    <div className="arbitrage-details">
      <Text className="H3 dark_1" text={"Arbitrage Details"} />
      ...
      <div className="arbitrage-details-content">
        <div className="table-title">
          <div className="table-title-item">Time</div>
          <div className="table-title-item">Amount of transaction</div>
          <div className="table-title-item">Earned Profit (USDT)</div>
          <div className="table-title-item">Status</div>
          <div className="table-title-item">Mode</div>
          <div className="table-title-item">Action</div>
        </div>
          ...
          {data.map((item, index) => (
            <div className="table-content-item">
              ...
              <div className="table-content-item-col">
                {item["status"] === "pending"
                  ? "-"
                  : Math.fround(item["u_earned"]).toFixed(3)}
              </div>
              <div className="table-content-item-col">
      ...
      <div className="arbitrage-details-footer">
        <Text
          ...
          text={`Showing 
              ${
                arbitrageDetails.pagination.pageSize *
                  (arbitrageDetails.pagination.page - 1) +
                1
              } 
              to 
              ${
                arbitrageDetails.pagination.pageSize *
                arbitrageDetails.pagination.page
              } 
            selected options`}
        />
        <Paginate
