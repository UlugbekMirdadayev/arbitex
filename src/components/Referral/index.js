...
const table_title = ["Partner", "Registration Date", "Email"];

const transaction_title = ["Date", "Amount", "Username", "Status"];
...
function Referral({
  ...
  useEffect(() => {
    ...
    getReferralUsers();
    getReferralTransactions();
  }, [user, getReferralInfo, getReferralUsers, getReferralTransactions]);
  ...
  return (
    <div className="referral">
      <div className="referral-header">
        ...
        <div className="referral">
          <Text className={"P5 dark_3"} text={"Referral link"} />
          <InputWithButton
            icon={"fa fa-link"}
            disabled
            text={referralLink}
            btnText={"copy"}
            onClick={() => {
              if (copyToClipboard(referralLink)) displayMsg("Copied");
            }}
          />
        </div>
      </div>
      <div className="referral-content">
        <Text className={"H5 dark_1"} text={"Your upline"} />
        ...
        <div className="referral-list">
          <Text className={"H3 dark_1 caption"} text={"Referral List"} />
          <div className="table-title">
            {table_title.map((item, index) => (
              <div key={index} className="table-title-item">
                {item}
              </div>
            ))}
          </div>
          <div className="table-content">
            {users.map((item, index) => (
              <div key={index} className="table-content-item">
                <div>{item.username}</div>
                <div>{displayDate(item.created_at)}</div>
                <div>{item.email}</div>
              </div>
            ))}
          </div>
        ...
        <div className="partner-transaction">
          <Text className={"H3 dark_1 caption"} text={"Partner Transaction"} />
          <div className="table-title">
            {transaction_title.map((item, index) => (
              <div className="table-title-item" key={index}>
                {item}
              </div>
            ))}
          </div>
...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    getReferralTransactions: () => {
      dispatch(getReferralTransactions());
    },
  };
};
