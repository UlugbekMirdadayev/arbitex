...
export function refactorBalances(balances) {
  ...
  BALANCEARRAYS.forEach((array) => {
    let balanceTemp;
    balances.userTotalBalances.forEach((utb) => {
    ...
    let withdrawTemp;
    balances.userTotalWithdraws.forEach((utw) => {
    ...
    let depositTemp;
    balances.userDepositBalances.forEach((udb) => {
...
export function calculateRealAvailableBalance(reflectBalanceState) {
  let data = [];
  reflectBalanceState.reflectuserBalances.forEach((currency, index) => {
    const balance =
