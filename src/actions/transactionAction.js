...
export const getDepositTransactions =
  ({ userId, page = 0, pageSize = 10, order = "DESC" }) =>
  async (dispatch) => {
    dispatch({
    ...
    try {
      const res = await apiServer.get(
        `deposits/${userId}?page=${page}&pageSize=${pageSize}&order=${order}`
      );
      dispatch({
...
export const createDepositFunds =
  ({ amount, userId, currency = "ERC20USDT" }) =>
  async (dispatch) => {
    dispatch({
...
export const getDepositHistory =
  ({ userId, page = 1, pageSize = 10, order = "DESC" }) =>
  async (dispatch) => {
    dispatch({
    ...
    try {
      const res = await apiServer.get(
        `/addfunds/${userId}?page=${page}&pageSize=${pageSize}&order=${order}`
      );
      dispatch({
...
export const getWithdrawHistory =
  ...
  async (dispatch) => {
    ...
    try {
      const res = await apiServer.get(
        `/withdraws/${userId}?page=${page}&pageSize=${pageSize}&order=${order}`
      );
      dispatch({
...
export const createOrder =
  ({ mode, userId, amount, currency, time, profit }) =>
  async (dispatch) => {
    dispatch({
